...
{
	public AmmoManager reference;
	//public int WeaponAmmoAmount;
	...
	public float knockBack = 10.0f;								//CUSTOM: Force dealt to the player when firing a weapon.
	...
	private void Awake()
	{
		//if (gameObject.tag == "Trench Club")
		{
			...
			//SMGSprite.SetActive(false);
			//LMGSprite.SetActive(false);
			//LauncherSprite.SetActive(false);
			//RocketGunSprite.SetActive(false);
			//FlamethrowerSprite.SetActive(false);
		}
	}
	...
	void Start()
	{
		if (reference != null)
		{
			reference.ammoAmount = reference.startingAmount;
		}
		...
		if (gameObject.tag == "Trench Club" || gameObject.tag == "Pistol" || gameObject.tag == "Revolver" || gameObject.tag == "Rifle" || gameObject.tag == "Shotgun" || gameObject.tag == "SMG" || gameObject.tag == "Lewis" || gameObject.tag == "Grenade Launcher" || gameObject.tag == "Rocket Gun" || gameObject.tag == "Flamethrower")
		{
	...
	void Update()
	{
		//if (reference != null)
		{
			//WeaponAmmoAmount = reference.ammoAmount;
		}

		reference.ammoAmount = currentAmmo;
	...
	void AmmoIcons()
	{
		if (reference.ammoAmount > 0)
		{
			reference.ammoAmount -= 1;
			//reference.ammo[reference.ammoAmount].gameObject.SetActive(false);
		}
	}
			...
			if (fireTimer >= actualROF && burstCounter < burstRate && canFire)
			{
				if (Input.GetButton("Fire1"))
				{
					if (!warmup)	// Normal firing when the user holds down the fire button
					{
						...
						//WeaponFired = true;
						//reference.ammoAmount -= 1;
						//reference.ammo[ammoAmount].gameObject.SetActive(false);
						AmmoIcons();
					}
	...
	void OnGUI()
	{
		...
		if (showCrosshair)
		{
			if (mask.masked == false)
            {
				...
				Rect centerRect = new Rect(new Rect(Screen.width / 2 - 25, Screen.height / 2 - 25, 60, 60));
				GUI.DrawTexture(centerRect, crosshairTexture, ScaleMode.StretchToFill);
			}
		}
